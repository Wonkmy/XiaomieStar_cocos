...
cc.Class({
    ...
    onLoad () {
        ...
        .repeatForever(
            cc.tween(this.progressBar.node.children[1].children[0])
            .by(0.5,{position:new cc.Vec3(0,this.progressBar.node.children[1].children[0].y+10,0)})
            .by(0.5,{position:new cc.Vec3(0,this.progressBar.node.children[1].children[0].y-10,0)})
        )
        .start()
        ...
        this.agreeYinsiNode.on(cc.Node.EventType.TOUCH_START,()=>{
        ...
        this.dontAgreeYinsiNode.on(cc.Node.EventType.TOUCH_START,()=>{
            cc.game.end();
        },this)

        this.settingPanelQuitGameBtn.on(cc.Node.EventType.TOUCH_START,()=>{
            cc.game.end();
        },this)
    ...
    setParent:function(){
        ...
        this.block_parent.y = -640 + this.block_parent.height / 2 + this.gameMapVerticalOffset//105
    ...
    addScore:function(num_score){
        ...
        if (this.node_success.active == false && this.numScore_curr >= this.getNextLevelTargetScore()) {
    ...
    getNextLevelTargetScore(){
        return this.numLevel*2000-1000
    },
    ...
    btn_callBack:function(sender,str){
       ...
       if (str == 'btn_play') {
        ...
        else if (str == 'BGM_btn_on') {
            this.bgm_is_sound = false
            this.shuaXinBGMBtn()
    ...
    shuaXinBGMBtn:function(){
        ...
        if (this.bgm_is_sound) {
            ...
            this.bgmNode.getComponent(cc.AudioSource).play()
            btn_on.active = true
    ...
    update (dt) {//1秒执行60次
        if(this.gameStart==false&&this.agreeYinsi==true){
            if(this.progressBar.progress<1){
                this.progressBar.progress+=dt/2
                this.SlideBar.progress+=dt/2
            }else{
                this.gameStart=true;
                this.progressBar.progress=1
                this.SlideBar.progress=1
